 ...
 * @mixin Builder
 */
class MiddleExamination extends Model
{
...
//        $log->type=OperationLog::TYPE_PROJECT_STARTUP['value'];
    ...
    public static function checkWithdrawPermission(User $user, MiddleExamination $project): bool
    {
        if($project->failed===true)return false;
        //为不同角色添加不同权限
        $student_permission = collect([MiddleExamination::STATUS_DRAFT,
            MiddleExamination::STATUS_SCHOOL_APPROVING])->pluck('value')->all();

        $school_permission = collect([MiddleExamination::STATUS_GRADUATE_SCHOOL_APPROVING])->pluck('value')->all();

        $graduate_school_permission=collect([MiddleExamination::STATUS_DONE])->pluck('value')->all();

        $permissions = $user->permissions()->pluck('code')->all();
        if (in_array(Permission::FLOW_MIDDLE_EXAMINATION_STUDENT['value'], $permissions)) {
            if (in_array($project->status, $student_permission)) {
                //只有本人才能撤销草稿
                if($project->status===MiddleExamination::STATUS_DRAFT['value']){
        ...
        if (in_array(Permission::FLOW_MIDDLE_EXAMINATION_SCHOOL['value'], $permissions)) {
            if (in_array($project->status, $school_permission)) {
                return true;
            }
        }
        if (in_array(Permission::FLOW_MIDDLE_EXAMINATION_GRADUATE_SCHOOL['value'], $permissions)) {
    ...
    public static function checkPassPermission(User $user, MiddleExamination $project): bool
    {
        if($project->failed===true)return false;
        //为不同角色添加不同权限
        $school_permission = collect([MiddleExamination::STATUS_SCHOOL_APPROVING])->pluck('value')->all();

        $graduate_school_permission=collect([MiddleExamination::STATUS_GRADUATE_SCHOOL_APPROVING])->pluck('value')->all();
