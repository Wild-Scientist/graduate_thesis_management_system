...
const CODE_STUDENT = 'flow.project_start_up.student';
const CODE_SCHOOL = 'flow.project_start_up.school';
const CODE_GRADUATE_SCHOOL = 'flow.project_start_up.graduate_school';
...
export function isWithdrawDisabled(currentUser, record) {
  ...
  const school_permissions = [
    CONST.STATUS_GRADUATE_SCHOOL_APPROVING['value'],
    CONST.STATUS_GRADUATE_SCHOOL_EXAMINING['value'],
  ];
  const graduate_school_permissions = [
    CONST.STATUS_SCHOOL_EXAMINING['value'],
    CONST.STATUS_CONTRACT_SIGNING['value'],
    CONST.STATUS_DONE['value'],
  ];
...
export function isPassDisable(currentUser, record) {
  if (record.failed === 1) return true;
  const school_permissions = [
    CONST.STATUS_SCHOOL_APPROVING['value'],
    CONST.STATUS_SCHOOL_EXAMINING['value'],
  ];
  const graduate_school_permissions = [
    CONST.STATUS_GRADUATE_SCHOOL_APPROVING['value'],
    CONST.STATUS_GRADUATE_SCHOOL_EXAMINING['value'],
    CONST.STATUS_GRADUATE_SCHOOL_CONTRACT_EXAMINING['value'],
  ];
  const status = record.status;
  if (currentUser?.permission_codes.includes(CODE_SCHOOL)) {
    if (school_permissions.includes(status)) return false;
...
export function userCanSeeContract(currentUser, record) {
  console.log(record);
  if (record.status === CONST.STATUS_CONTRACT_SIGNING['value']) return true;
  return false;
}
